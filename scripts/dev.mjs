...
// Bind Next on 0.0.0.0 so the Parallels Windows VM running NinjaTrader can
// reach our /api/nt8/* routes when Local Mode is active. Default `next dev`
// listens only on localhost, which is unreachable from the VM. The host
// stays at the same port (3000) — the user pastes
// http://10.211.55.2:3000 (Parallels Shared gateway IP) into Settings.
start("next",        ["dev:next", "--", "-H", "0.0.0.0"]);
