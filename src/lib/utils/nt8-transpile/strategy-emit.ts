...
import { synthesizeLegacyFilters } from "./legacy-filter-synth";
...
export function transpileDslToCs(input: TranspileInput): TranspileResult {
  ...
  for (const s of parsed.stmts) {
    ...
    if (s.kind === "assign") {
      ...
      // `filters.<family>.<key>` legacy preset filters are picked up by
      // `mergeScriptFilters` below (regex scan over the raw script) and
      // either fold into PresetFiltersData (time/trend) or are converted
      // into synthesized filter.if directives via legacy-filter-synth.ts.
      // Don't warn here — the parallel path handles them.
      if (s.path.startsWith("filters.")) continue;
  ...
  // Synthesize filter.if directives from the legacy preset filters
  // (filters.adx.*, filters.rsi.*, etc.) that NT8 has no native
  // PresetFiltersData support for. Reproduces preset-filters.ts
  // semantics by building Expr ASTs that the existing filter.if emit
  // pipeline turns into identical-semantics C#. See legacy-filter-synth.ts.
  //
  // Synthesized directives sit AFTER user-authored ones in the
  // filterIfDirectives list so user filter.if rule overrides win when
  // both happen to fire on the same bar; AND-gating semantics are
  // unaffected by ordering.
  //
  // filters.time.* and filters.trend.* are intentionally NOT synthesized
  // here — they stay on the native PresetFiltersData path below.
  const synthesizedDirectives = synthesizeLegacyFilters(mergedFilters);
  for (const directive of synthesizedDirectives) {
    filterIfDirectives.push({ scope: "both", directive });
  }
  ...
  const csSource = renderStrategyFile({
    ...
    synthesizedFilterCount: synthesizedDirectives.length,
    ctx,
    warnings,
  });
...
interface RenderArgs {
  ...
  scriptText: string;
  /** How many filter.if directives the legacy-filter synthesizer
   *  produced from `filters.<family>.<key>` lines. Surfaces in the
   *  header diagnostic so we can confirm at a glance that legacy
   *  preset filters are reaching NT8 (a non-zero count means parity
   *  with the dashboard's preset-filters.ts evaluator). */
  synthesizedFilterCount: number;
  ctx: EmitContext;
  warnings: string[];
}
...
// Synthesized ${a.synthesizedFilterCount} filter.if directive(s) from legacy filters.* (adx/atr/rsi/bollinger/bbWidth/maDistance/volume/adxTrend/delta).
${paramsLine}
...
function mergeScriptFilters(merged: Record<string, unknown>, scriptText: string): void {
  ...
  for (const rawLine of lines) {
    ...
    // Generic catch-all for the remaining filter families (adx, atr,
    // rsi, bollinger, bbWidth, maDistance, volume, adxTrend, delta).
    // These don't land in PresetFiltersData on the NT8 side — instead
    // legacy-filter-synth.ts converts them to filter.if directives at
    // emit time. We just need each `filters.<family>.<key>` line to
    // populate `merged[family][key]` so the synthesizer can read it.
    else {
      const m2 = path.match(/^filters\.([a-zA-Z][a-zA-Z0-9]*)\.([a-zA-Z][a-zA-Z0-9.]*)$/);
      if (m2) {
        const family = m2[1];
        const key = m2[2];
        // Only route the families the synthesizer knows about so a typo
        // like `filters.bogus.x` doesn't quietly create a phantom block.
        const KNOWN = new Set([
          "adx", "atr", "rsi",
          "bollinger", "bbWidth",
          "maDistance", "volume", "adxTrend", "delta",
        ]);
        if (KNOWN.has(family)) {
          // Booleans / numbers / strings / arrays all flow through as-is;
          // the synthesizer's numField/strField/isEnabled helpers coerce
          // permissively at use site.
          ensureBlock(family)[key] = value;
        }
      }
    }
  }
