...
rules.breakEvenTrigger = 3
rules.cooldownBetweenTradesBars = 10

rules.scalingEnabled = false
rules.scalingStartSize = 1
rules.scalingWinStep = 1
rules.scalingLossStep = 2
rules.scalingMaxSize = 5

rules.slAtrAdjust = 5
rules.tpAtrAdjust = 4.5

rules.timedExitBars = 60
rules.timedExitEnabled = true

filters.time.enabled = true
filters.time.windows = ["08:00-14:00"]

filters.trend.enabled = true
filters.trend.ema20 = against
filters.trend.ema200 = any

filter.if = (volume > 300, , )
// filter.if = (ADX >= 15, , )
// filter.if = (EMA200 > close, , )
// filter.if = (ATR(14) >= 3, , )
